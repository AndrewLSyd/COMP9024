...
// the data structure is an array of height HEIGHT and a pointer which keeps track of the top of the stack
struct node {
   int array[HEIGHT];
   int top;    // used by both pop and push
};

// Quack type is a pointer to a node
Quack createQuack(void) {
   ...
   qs->top = -1;  // nothing in the stack right now, set it to -1
   return qs;
}
...
void qush(int data, Quack qs) {
   // instead of push where we push to the top of the quack, we need to push to the front of the quack
   if (qs == NULL) {
      fprintf(stderr, "push: quack not initialised\n");
   }
   else {
      if (qs->top >= HEIGHT-1) {
         ...
         ++qs->top; // same as ++(qs->top); same as (*qs).top = (*qs).top + 1
         // move everything forward by one
         int temp_1 = data, temp_2;
         for (int i = 0; i <= qs->top; i++){
            temp_2 = qs->array[i];
            qs->array[i] = temp_1;
            temp_1 = temp_2;
         }
